    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 mb-5 flex flex-wrap gap-4">
        <div class="flex-1 min-w-[200px]">
          ...
          <select v-model="filters.customer_id" class="form-input text-sm">
        ...
        <div class="flex items-end gap-2">
          ...
          <a v-if="filters.customer_id" :href="exportUrl" target="_blank"
      ...
      <div v-if="!filters.customer_id" class="bg-white rounded-xl shadow-sm p-12 text-center text-gray-400">
        Chọn khách hàng để xem sổ chi tiết công nợ phải thu
      </div>
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-3 gap-4 mb-5">
          <div class="bg-white rounded-xl shadow-sm p-4">
            <p class="text-xs text-gray-500 mb-1">Dư đầu kỳ (Nợ)</p>
            <p class="text-lg font-bold" :class="opening_bal >= 0 ? 'text-blue-600' : 'text-red-600'">
              {{ formatVnd(Math.abs(opening_bal)) }}
              <span class="text-xs font-normal text-gray-400 ml-1">{{ opening_bal >= 0 ? 'Nợ' : 'Có' }}</span>
            </p>
          </div>
          <div class="bg-white rounded-xl shadow-sm p-4">
          ...
          <div class="bg-white rounded-xl shadow-sm p-4">
            <p class="text-xs text-gray-500 mb-1">Dư cuối kỳ (Nợ)</p>
            <p class="text-lg font-bold" :class="closing_bal >= 0 ? 'text-blue-600' : 'text-red-600'">
              {{ formatVnd(Math.abs(closing_bal)) }}
              <span class="text-xs font-normal text-gray-400 ml-1">{{ closing_bal >= 0 ? 'Nợ' : 'Có' }}</span>
            </p>
          </div>
        </div>
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-600 uppercase text-xs">
              <tr>
                ...
                <th class="px-4 py-3 text-right w-32">Dư Nợ</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              ...
              <tr class="bg-blue-50 font-medium">
                ...
                <td class="px-4 py-2 text-blue-700">Số dư đầu kỳ</td>
              ...
              <tr v-for="(row, i) in rows" :key="i" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2 text-right font-medium" :class="row.balance >= 0 ? 'text-blue-700' : 'text-red-600'">
              ...
              <tr class="bg-gray-50 font-semibold border-t-2 border-gray-300">
                ...
                <td class="px-4 py-2 text-right text-blue-700">{{ formatVnd(closing_bal) }}</td>
              </tr>
...
function applyFilters() {
  router.get(route('reports.ar.detail'), filters.value, { preserveState: true });
}

const exportUrl = computed(() => {
  const p = new URLSearchParams();
  Object.entries(filters.value).forEach(([k, v]) => { if (v) p.set(k, v); });
  return route('reports.ar.detail.export') + '?' + p.toString();
});
