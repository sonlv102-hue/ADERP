  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Công nợ phải thu (AR)</h1>
          <p class="text-sm text-gray-500 mt-0.5">Hóa đơn bán — tổng tiền, đã thu, còn lại theo hạn thanh toán</p>
        </div>
      ...
      <div class="flex gap-3 flex-wrap items-center">
        <input v-model="search" @keyup.enter="applyFilters" type="text" placeholder="Mã HĐ, khách hàng..."
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th class="text-left px-4 py-3 font-semibold text-gray-600">Ngày xuất</th>
              ...
              <th class="text-right px-4 py-3 font-semibold text-gray-600">Đã thu</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="row in rows.data" :key="row.id" class="hover:bg-gray-50">
              <td class="px-4 py-3">
                <Link :href="route('accounting.invoices.show', row.id)"
              ...
              <td class="px-4 py-3 text-gray-500 text-xs">{{ fmtDate(row.issue_date) }}</td>
...
const exportUrl = computed(() => {
  ...
  return route('reports.ar.aging.export') + (qs ? '?' + qs : '');
});

function applyFilters() {
  router.get(route('reports.ar.aging'), {
