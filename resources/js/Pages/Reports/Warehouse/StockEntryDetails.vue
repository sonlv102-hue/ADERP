  ...
  <AppLayout>
    <div class="space-y-5">
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Báo cáo chi tiết nhập kho</h1>
          <p class="text-sm text-gray-500 mt-0.5">Xem hàng hóa nhập ngày nào, nhập ở kho nào và số lượng bao nhiêu.</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div>
          ...
          <input type="text" v-model="filters.search" @keyup.enter="applyFilters" placeholder="Mã SP, tên SP, mã NK..." class="form-input mt-1 w-full" />
        </div>
      </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-if="!rows.data.length">
              <td colspan="10" class="px-4 py-10 text-center text-gray-400">Không có dữ liệu phù hợp</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const exportUrl = computed(() => {
  ...
  return route('reports.stock_entry_details.export') + (params.toString() ? `?${params.toString()}` : '');
});

function applyFilters() {
  router.get(route('reports.stock_entry_details'), {
