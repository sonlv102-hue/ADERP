      ...
      <div v-if="can('accounting.manage')" class="bg-white rounded-xl shadow-sm p-5 mb-5">
        ...
        <form @submit.prevent="submitAdd" class="grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
          ...
          <div>
            ...
            <input v-model.number="addForm.credit" type="number" min="0" step="1000" class="form-input text-sm" placeholder="0" />
          </div>
          <div>
            <label class="form-label text-xs">Số tiền ra (Nợ)</label>
            <input v-model.number="addForm.debit" type="number" min="0" step="1000" class="form-input text-sm" placeholder="0" />
          </div>
          <div>
