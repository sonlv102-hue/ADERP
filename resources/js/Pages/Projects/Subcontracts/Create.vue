  ...
  <AppLayout>
    <div class="max-w-4xl space-y-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Thêm hợp đồng khoán</h1>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div v-for="ct in contractorTypes" :key="ct.value"
            ...
            <p class="text-xs text-gray-500 mt-0.5">{{ ct.description }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <p v-if="form.errors.contract_no" class="text-red-500 text-xs mt-1">{{ form.errors.contract_no }}</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ngày ký <span class="text-red-500">*</span></label>
          ...
          <div v-if="form.contractor_type === 'company'">
            <label class="block text-sm font-medium text-gray-700 mb-1">Nhà cung cấp / Nhà thầu</label>
            <RemoteSearchSelect
              v-model="form.contractor_id"
              :search-url="route('search.suppliers')"
              :display-text="form.contractor_name"
              placeholder="Tìm theo tên, mã NCC..."
              @change="(opt) => { form.contractor_name = opt ? opt.label : form.contractor_name }"
            />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tên nhà thầu / đội nhóm <span class="text-red-500">*</span></label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Giá trị trước VAT <span class="text-red-500">*</span></label>
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-gray-100">
          <Link :href="route('projects.projects.show', project.id)" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Hủy</Link>
          <button type="button" @click="submit" :disabled="form.processing"
            ...
            {{ form.processing ? 'Đang xử lý...' : 'Tạo hợp đồng' }}
          </button>
        </div>
      </div>
    </div>
...
const contractorTypes = [
  { value: 'company',    label: 'Có hóa đơn (pháp nhân)', description: 'Nghiệm thu → Nợ 154 / Nợ 1331 / Có 3312' },
  { value: 'team',       label: 'Đội nhóm', description: 'Không hóa đơn, không VAT → Nợ 154 / Có 3388' },
  { value: 'individual', label: 'Cá nhân', description: 'Không hóa đơn, không VAT → Nợ 154 / Có 3388' },
];
...
function submit() {
  form.post(route('projects.projects.subcontracts.store', props.project.id));
}
</script>
