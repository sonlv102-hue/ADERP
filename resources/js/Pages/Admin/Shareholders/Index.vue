  ...
  <AppLayout>
    <div class="max-w-5xl space-y-5">
      <div class="flex items-center justify-between flex-wrap gap-3">
        <h1 class="text-2xl font-bold text-gray-900">Thành viên / Cổ đông</h1>
        <Link :href="route('admin.shareholders.create')"
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th class="text-left px-4 py-3 font-medium text-gray-600">CCCD/CMND</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Điện thoại</th>
              <th class="text-right px-4 py-3 font-medium text-gray-600">% Vốn góp</th>
              <th class="text-center px-4 py-3 font-medium text-gray-600">Trạng thái</th>
              <th class="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-if="shareholders.data.length === 0">
              <td colspan="7" class="px-4 py-8 text-center text-gray-400">Chưa có thành viên nào.</td>
            </tr>
            <tr v-for="s in shareholders.data" :key="s.id" class="hover:bg-gray-50">
              <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ s.code }}</td>
              <td class="px-4 py-3 font-medium text-gray-900">{{ s.name }}</td>
              <td class="px-4 py-3 text-gray-600">{{ s.id_number || '—' }}</td>
              <td class="px-4 py-3 text-gray-600">{{ s.phone || '—' }}</td>
              <td class="px-4 py-3 text-right text-gray-600">
                {{ s.share_percentage != null ? s.share_percentage + '%' : '—' }}
              </td>
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex items-center gap-2 justify-end">
                  <Link :href="route('admin.shareholders.edit', s.id)"
                    class="text-primary-600 hover:underline text-xs">Sửa</Link>
                  <button @click="confirmDelete(s)" class="text-red-500 hover:underline text-xs">Xóa</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Pagination -->
      <div v-if="shareholders.last_page > 1" class="flex gap-2">
        <Link v-for="link in shareholders.links" :key="link.label"
          :href="link.url || '#'"
    ...
    <div v-if="deleteTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl space-y-4">
        <h3 class="font-semibold text-gray-900">Xóa thành viên?</h3>
        <p class="text-sm text-gray-600">Thành viên <strong>{{ deleteTarget.name }}</strong> sẽ bị xóa.</p>
...
const props = defineProps({ shareholders: Object, filters: Object });

const searchInput = ref(props.filters?.search || '');
const deleteTarget = ref(null);

function applyFilters() {
  router.get(route('admin.shareholders.index'), { search: searchInput.value }, { preserveState: true, replace: true });
}
function clearFilters() {
  searchInput.value = '';
  router.get(route('admin.shareholders.index'));
}
function confirmDelete(s) { deleteTarget.value = s; }
function doDelete() {
  router.delete(route('admin.shareholders.destroy', deleteTarget.value.id), {
