  ...
  <AppLayout>
    <div class="max-w-5xl space-y-5">
      <div class="flex items-center justify-between flex-wrap gap-3">
        <h1 class="text-2xl font-bold text-gray-900">Quản lý chức vụ</h1>
      ...
      <div class="bg-white rounded-xl border border-gray-200 overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Mã chức vụ</th>
              <th class="text-left px-4 py-3 font-medium text-gray-600">Tên chức vụ</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-if="positions.data.length === 0">
              <td colspan="5" class="px-4 py-8 text-center text-gray-400">Chưa có chức vụ nào.</td>
            </tr>
            <tr v-for="p in positions.data" :key="p.id" class="hover:bg-gray-50">
              <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ p.code }}</td>
              <td class="px-4 py-3 font-medium text-gray-900">{{ p.name }}</td>
              <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ p.notes || '—' }}</td>
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex items-center gap-2 justify-end">
                  ...
                  <button @click="confirmDelete(p)" class="text-red-500 hover:underline text-xs">Xóa</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Pagination -->
      <div v-if="positions.last_page > 1" class="flex gap-2">
        <Link v-for="link in positions.links" :key="link.label"
          :href="link.url || '#'"
    ...
    <div v-if="deleteTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 max-w-sm w-full mx-4 shadow-xl space-y-4">
        <h3 class="font-semibold text-gray-900">Xóa chức vụ?</h3>
        <p class="text-sm text-gray-600">Chức vụ <strong>{{ deleteTarget.name }}</strong> sẽ bị xóa.</p>
...
function applyFilters() {
  router.get(route('admin.positions.index'), { search: searchInput.value }, { preserveState: true, replace: true });
}
...
function doDelete() {
  router.delete(route('admin.positions.destroy', deleteTarget.value.id), {
