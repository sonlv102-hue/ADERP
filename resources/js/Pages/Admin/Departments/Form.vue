  ...
  <AppLayout>
    <div class="max-w-lg">
      <div class="flex items-center gap-3 mb-6">
        <Link :href="route('admin.departments.index')" class="text-gray-500 hover:text-gray-700">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <h1 class="text-2xl font-bold text-gray-900">{{ department ? 'Sửa bộ phận' : 'Thêm bộ phận' }}</h1>
      </div>
      ...
      <form @submit.prevent="submit" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div class="flex gap-3 pt-2">
          <button type="submit" :disabled="form.processing"
            ...
            {{ form.processing ? 'Đang lưu...' : (department ? 'Cập nhật' : 'Thêm mới') }}
          </button>
