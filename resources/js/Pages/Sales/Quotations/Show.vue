  ...
  <AppLayout>
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th v-if="hasRole('admin')" class="px-5 py-3"></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(item, i) in quotation.items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td v-if="hasRole('admin')" class="px-5 py-3 text-right whitespace-nowrap">
                <button v-if="quotation.status === 'cancelled'"
    ...
    <Modal :show="showFixProductModal" max-width="lg" @close="showFixProductModal = false">
      ...
      <div class="space-y-4">
        ...
        <p v-if="quotation.orders?.length" class="text-xs text-slate-500">
          Báo giá này đã được chuyển thành đơn hàng — sửa dòng này không ảnh hưởng đơn hàng đã tạo.
        </p>
        ...
        <FormField label="Lý do sửa" required>
          <textarea v-model="fixProductForm.reason" rows="2" maxlength="255" class="erp-input"
            placeholder="VD: Báo giá gắn nhầm sản phẩm khi tạo, sản phẩm đúng là..."></textarea>
        </FormField>
      </div>
      <template #footer>
...
function submitFixProduct() {
  ...
  router.post(
    route('sales.quotations.items.fix-product', [props.quotation.id, fixProductTarget.value.id]),
