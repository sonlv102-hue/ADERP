    ...
    <div class="max-w-5xl space-y-5">
      ...
      <div v-if="order.customer.is_fdi" class="rounded-xl border p-5 space-y-3"
        ...
        <div v-else class="space-y-3">
          ...
          <div class="space-y-2">
            <div>
              <label class="block text-xs font-medium text-amber-700 mb-1">Ngày khai báo</label>
              <input type="datetime-local" v-model="customsDate"
      ...
      <FileAttachments
        ...
        :upload-url="route('attachments.store', { type: 'order', id: order.id })"
      />
...
const customsDate = ref(new Date().toISOString().slice(0, 16));
...
const submitCustoms = () => {
  ...
  if (customsDate.value) formData.append('customs_declared_at', customsDate.value);
