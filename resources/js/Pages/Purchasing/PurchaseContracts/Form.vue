  ...
  <AppLayout>
    <div class="max-w-3xl">
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">{{ isEdit ? 'Sửa hợp đồng mua' : 'Tạo hợp đồng mua' }}</h1>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-5">
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <!-- Đơn mua liên kết -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Đơn mua hàng liên kết</label>
              ...
              <p v-if="!form.supplier_id" class="mt-1 text-xs text-gray-400">Chọn nhà cung cấp trước</p>
              <p v-else-if="filteredOrders.length === 0" class="mt-1 text-xs text-gray-400">NCC này chưa có đơn mua</p>
            </div>
        ...
        <div class="flex gap-3">
          ...
          <Link :href="route('purchasing.purchase-contracts.index')"
...
const form = useForm({
  ...
  supplier_name:      props.contract?.supplier_name ?? props.contract?.supplier?.name ?? '',
  purchase_order_id:  props.contract?.purchase_order_id ?? null,
...
const filteredOrders = computed(() => {
  ...
  return props.orders.filter(o => o.supplier_id == form.supplier_id);
});

watch(() => form.supplier_id, () => {
  const belongs = filteredOrders.value.some(o => o.id === form.purchase_order_id);
  if (!belongs) form.purchase_order_id = null;
});

watch(() => form.purchase_order_id, (newId) => {
  if (newId) {
    const order = props.orders.find(o => o.id === newId);
    if (order?.total) form.value = order.total;
  }
});

const submit = () => {
  if (isEdit) {
    form.put(route('purchasing.purchase-contracts.update', props.contract.id));
  } else {
    form.post(route('purchasing.purchase-contracts.store'));
  }
};
</script>
