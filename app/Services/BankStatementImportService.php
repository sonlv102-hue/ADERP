        ...
        try {
            ...
            for ($i = $headerIndex + 1; $i < count($rows); $i++) {
                ...
                BankStatementImportRow::create([
                    ...
                    'raw_data_json'               => [
                        'tx_type'                  => $data['tx_type'] ?? 'unknown',
                        'internal_account_id'      => $data['internal_account_id'] ?? null,
                        'supplier_bank_account_id' => $data['supplier_bank_account_id'] ?? null,
                        'customer_bank_account_id' => $data['customer_bank_account_id'] ?? null,
        ...
        DB::transaction(function () use ($validRows, $account, $batch, $userId, &$imported) {
            foreach ($validRows as $row) {
                ...
                $tx = $account->transactions()->create([
                    ...
                    'internal_account_id'      => $row->raw_data_json['internal_account_id'] ?? null,
                    'supplier_bank_account_id' => $row->raw_data_json['supplier_bank_account_id'] ?? null,
                    'customer_bank_account_id' => $row->raw_data_json['customer_bank_account_id'] ?? null,
                    'alert_note'               => $row->raw_data_json['alert_note'] ?? null,
